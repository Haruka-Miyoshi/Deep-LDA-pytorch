...
class DeepLDA(object):
    def __init__(self, x_dim:int, h_dim:int, K:int, lr:float=3e-4, train_mode:bool=True, save_path="param", model_path="deep_lda_parameter.path"):
        ...
        self.model = Model(self.x_dim, self.h_dim, self.K).to(device=self.device)
    ...
    def train(self, train_loader, epoch=30):
        ...
        for e in range(1, epoch + 1):
            ...
            for i, (batch_data, label) in enumerate(train_loader):
                ...
                mu, logvar, z, theta, xh = self.model(x, True)
                loss = ( self.KLD(mu, logvar) + self.BCE(xh, x) ).mean()
                loss.backward()
    ...
    def x_to_z_to_xh(self, x):
        ...
        with torch.no_grad():
            mu, logvar, z, theta, xh = self.model(x, False)
        return mu, logvar, z, theta, xh
    
    def x_to_theta(self, x):
        self.model.eval() # 推論モード        
        with torch.no_grad():
            mu, logvar, z, theta = self.model.encode(x, False)
        return mu, logvar, z, theta
    
    def theta_to_x(self, theta):
        self.model.eval() # 推論モード        
        with torch.no_grad():
            xh = self.model.decode(theta, False)
        return xh